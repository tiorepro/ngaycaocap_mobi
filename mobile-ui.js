...
const MOBILE_STATE = {
    ...
    selectedHours: {},
    filterState: null,
    chonCanArr: [], // Lưu kết quả Can nên chọn từ bảng Tổng Hợp
    chonChiArr: []  // Lưu kết quả Chi nên chọn từ bảng Tổng Hợp
};

// ==================== HỖ TRỢ HIỂN THỊ QUẺ HKĐQ ====================
// Map 8 quẻ Đơn (Trigram) để xếp chồng thành quẻ 6 hào
const TRIGRAM_MAP = {
    'Thiên': '☰', 'Trạch': '☱', 'Hỏa': '☲', 'Lôi': '☳',
    'Phong': '☴', 'Thủy': '☵', 'Sơn': '☶', 'Địa': '☷',
    'Càn': '☰', 'Đoài': '☱', 'Ly': '☲', 'Chấn': '☳',
...
function renderGuaVisual(canChi, hanhArr, vanArr) {
    ...
    for (let i = 0; i < ques.length; i++) {
        ...
        const stackHTML = getGuaStackHTML(qName);
        ...
        <div class="m-gua-block">
            ...
            <div class="m-gua-name">${qName}</div>
        </div>`;
    }
    html += '</div>';
    return html;
...
function toggleMonthDropdown() {
    ...
    const chevron = document.getElementById('month-dropdown-chevron');
    if (!body) return;
    if (body.style.display === 'none') {
        body.style.display = 'block';
        chevron.textContent = '▲';
...
// ==================== CHỌN THÁNG (PHẠM VI XEM) ====================
const MOBILE_MONTH_NAMES = [
    'Tháng Giêng', 'Tháng 2', 'Tháng 3', 'Tháng 4', 'Tháng 5', 'Tháng 6',
    'Tháng 7', 'Tháng 8', 'Tháng 9', 'Tháng 10', 'Tháng 11', 'Tháng Chạp',
];
...
function buildMonthGrid() {
    ...
    grid.innerHTML = MOBILE_MONTH_NAMES.map((name, i) => {
        ...
        // Xác định Địa chi của tháng tương ứng (Tháng 1 là Dần, ...) từ mảng tietKhiMonthChi trong constants.js
        const chiOfMonth = tietKhiMonthChi[m - 1]; 
        
        // Thêm biểu tượng sao nếu Chi tháng này nằm trong danh sách Chọn
        const star = chonChis.includes(chiOfMonth) ? '<span class="m-chip-star" title="Nên chọn">⭐</span>' : '';
...
async function handleViewResult() {
    ...
    try {
        ...
        renderInputCards();
        // Cần gọi renderTranhChonSummary trước để nó sinh ra mảng MOBILE_STATE.chonCanArr và chonChiArr
...
function renderTranhChonSummary() {
    ...
    const chonRows = [
        ...
        { label: 'THB Long - Ấn Cục', nam: cell([], []), thang: cell([], thblChiAn), ngay: cell(thblCan ? [thblCan] : [], thblChiAn), gio: cell(thblCan ? [thblCan] : [], thblChiAn) },
        { label: 'THB Long - Tài Cục', nam: cell([], []), thang: cell([], thblChiTai), ngay: cell(thblCan ? [thblCan] : [], thblChiTai), gio: cell(thblCan ? [thblCan] : [], thblChiTai) },
        { label: 'THB Long - Vượng Cục', nam: cell([], []), thang: cell([], thblChiVuong), ngay: cell(thblCan ? [thblCan] : [], thblChiVuong), gio: cell(thblCan ? [thblCan] : [], thblChiVuong) },
    ];
    ...
    html += `<thead>
        <tr>
            <th class="col-type" rowspan="2">MỤC</th>
    ...
    html += `<tr class="tc-summary-row tc-tranh">
                <td class="col-type" style="color:var(--danger);">🚫 TRÁNH</td>`;
    cols.forEach(col => { html += renderCellPair(ketLuanTranh[col], 'tranh'); });
    html += `</tr>`;

    html += `<tr class="tc-summary-row tc-chon">
                <td class="col-type" style="color:var(--success);">✅ CHỌN</td>`;
...
function createFilterUI() {
    buildMonthGrid(); // 1A
    createFilterCanChi(); // 1B
    createFilterTietKhi(); // 1C
...
function updateFilterUIWithData() {
    buildMonthGrid(); // Cập nhật lại sao ⭐
    createFilterCanChi(); // Cập nhật lại sao ⭐
    createFilterTietKhi(); // Cập nhật lại sao ⭐
}
...
function createFilterCanChi() {
    ...
    const chonCans = MOBILE_STATE.chonCanArr || [];
    const chonChis = MOBILE_STATE.chonChiArr || [];

    if (canContainer) {
        canContainer.innerHTML = THIEN_CAN.map(c => {
            const star = chonCans.includes(c) ? '<span class="m-chip-star" title="Nên chọn">⭐</span>' : '';
    ...
    if (chiContainer) {
        chiContainer.innerHTML = DIA_CHI.map(c => {
            const star = chonChis.includes(c) ? '<span class="m-chip-star" title="Nên chọn">⭐</span>' : '';
...
function createFilterHanhVanPairs() {
    ...
    if (hanhContainer) hanhContainer.innerHTML = buildPairHTML(pairLabels, hanhRelations, 'hanh');
    if (vanContainer) vanContainer.innerHTML = buildPairHTML(pairLabels, vanRelations, 'van');
}
    ...
    requestAnimationFrame(async () => {
        try {
            ...
            for (const dateInfo of MOBILE_STATE.allDates) {
                ...
                // Lọc Tiết khí Tầng 1 (Chỉ xét cho Ngày)
                ...
                for (const h of hours) {
                    ...
                    // ==== LỌC CAN / CHI (1B) - Áp dụng cho Can/Chi Ngày hoặc Giờ ====
                    let skipHour = false;
                    
                    if (filterState.can.length > 0) {
                        // Nếu Can Ngày không khớp VÀ Can Giờ không khớp => Bỏ qua giờ này
                        if (!filterState.can.includes(dayCan) && !filterState.can.includes(hourCan)) {
                            skipHour = true;
                        }
                    }
                    if (filterState.chi.length > 0) {
                        // Nếu Chi Ngày không khớp VÀ Chi Giờ không khớp => Bỏ qua giờ này
