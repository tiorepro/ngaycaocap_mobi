...
function renderTranhChonSummary() {
    ...
    const tranhRows = [
        ...
        { label: 'Xung Thái Tuế', nam: cell([], [tuePhaChi]), thang: cell([], [tuePhaChi]), ngay: cell([], [tuePhaChi]), gio: cell([], [tuePhaChi]) },
    ...
    html += `<tr class="tc-summary-row tc-tranh">
                <td class="col-type" style="color:var(--danger);">🚫 KẾT LUẬN TRÁNH</td>`;
    cols.forEach(col => { html += renderCellPair(ketLuanTranh[col], 'tranh'); });
    html += `</tr>`;

    html += `<tr class="tc-summary-row tc-chon">
                <td class="col-type" style="color:var(--success);">✅ KẾT LUẬN CHỌN</td>`;
    ...
    requestAnimationFrame(async () => {
        try {
            ...
            for (const dateInfo of MOBILE_STATE.allDates) {
                ...
                // Lọc Can/Chi Tầng 1
                if (filterState.can.length > 0 && !filterState.can.includes(dayCan)) continue;
                if (filterState.chi.length > 0 && !filterState.chi.includes(dayChi)) continue;
                ...
                for (const h of hours) {
                    ...
                    if (filterState.chi.length > 0 && !filterState.chi.includes(hourChi)) continue;
